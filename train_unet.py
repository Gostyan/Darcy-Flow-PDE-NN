"""
train_unet.py — U-Net baseline for 2D Darcy Flow prediction.

Usage:
    python train_unet.py
    python train_unet.py --epochs 200 --batch_size 16 --lr 1e-3
    python train_unet.py --pretrain_path pretrain_model/unet/DarcyFlow_Unet.tar
"""
    ...
    for k, v in state_dict.items():
        ...
        for old, new in prefix_map.items():
            if new_k.startswith(old):
                ...
                break
        # final 1×1 conv: pretrain uses 'conv.*', our model uses 'out_conv.*'
...
def get_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train U-Net on 2D Darcy Flow")
    ...
    p.add_argument("--reduced_res",   type=int,   default=1,
                   help="Spatial sub-sampling factor (1 = full resolution)")
    p.add_argument("--num_samples",   type=int,   default=-1)
    p.add_argument("--checkpoint_dir",type=str,   default="checkpoints/unet")
...
def train(args: argparse.Namespace) -> None:
    ...
    no_improve  = 0  # early-stopping counter (in evaluation units)
